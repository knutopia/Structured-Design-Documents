...
function build(proof: any, historicalRegularNodes = false) {
  ...
  // Replay the historical geometry with its original regular-node metrics.
  class HistoricalSceneBuilder extends UiContractsSceneBuilder {
    override node(occurrence: Parameters<UiContractsSceneBuilder["node"]>[0]) {
      const node = super.node(occurrence);
      if (node.sharedNode) delete node.sharedNode.emphasized;
      return node;
    }
  }
  const Builder = historicalRegularNodes ? HistoricalSceneBuilder : UiContractsSceneBuilder;
  const builder = new Builder(proof.detail, { id: proof.decorators, showNodeType: proof.decorators.includes("type"), showNodeId: proof.decorators.includes("id") });
...
const geometry = (scene: PositionedScene) => ({
  ...
  edges: scene.edges.map(edge => ({ route: edge.route.points, label: edge.label,
    markers: Object.values(edge.markers ?? {}).some(Boolean) ? edge.markers : undefined, classes: edge.classes }))
    .sort((a, b) => JSON.stringify(a).localeCompare(JSON.stringify(b)))
});

describe("production scene builders reproduce B5 scopes", () => {
  it("validates emphasized scopes and preserves historical B5 geometry with regular-node metrics", async () => {
    for (const proof of evidence.proofs) {
      ...
      const historical = await runStagedRendererPipeline(build(proof, true).scene);
      const actual = geometry(historical.positionedScene);
