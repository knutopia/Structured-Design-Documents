...
async function buildIaArtifacts(examplePath: string, profileId: string) {
  ...
  const rendererScene = buildIaPlaceMapRendererScene(projected.projection, compiled.graph, view, { profileId: profileId, detailId: profileId === "simple" ? "compact" : "detailed" });
  const rendered = await renderIaPlaceMapStagedSvg(projected.projection, compiled.graph, view, { profileId: profileId, detailId: profileId === "simple" ? "compact" : "detailed" });
...
async function buildIaRendererSceneFromSource(sourceText: string, profileId = "simple"): Promise<RendererScene> {
  ...
  return buildIaPlaceMapRendererScene(projected.projection, compiled.graph, view, { profileId: profileId, detailId: profileId === "simple" ? "compact" : "detailed" });
}
