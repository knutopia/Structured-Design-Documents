...
const GATE8_TERMINAL_DIAGNOSTIC_CODES = new Set([
...
function withoutGate8TerminalDiagnostics(
  diagnostics: readonly RendererDiagnostic[]
): RendererDiagnostic[] {
  return diagnostics.filter((diagnostic) => !GATE8_TERMINAL_DIAGNOSTIC_CODES.has(diagnostic.code));
}

function withoutKnownDenseExperimentDebt(
  diagnostics: readonly RendererDiagnostic[]
): RendererDiagnostic[] {
...
describe("journey map Gate 5 basic routing", () => {
  it("balances direct coordinates symmetrically and resolves reservations deterministically", () => {
    expect(selectJourneyMapBalancedDirectCoordinate({
      source: { y: 92, height: 112 },
      target: { y: 92, height: 48 }
    })).toBe(116);
    expect(selectJourneyMapBalancedDirectCoordinate({
      source: { y: 92, height: 48 },
      target: { y: 92, height: 112 }
    })).toBe(116);
    expect(selectJourneyMapBalancedDirectCoordinate({
      source: { y: 92, height: 64 },
      target: { y: 100, height: 64 }
    })).toBe(128);
    expect(selectJourneyMapBalancedDirectCoordinate({
      source: { y: 92, height: 112 },
      target: { y: 92, height: 48 },
      sourceReservations: [116]
    })).toBe(132);
    expect(selectJourneyMapBalancedDirectCoordinate({
      source: { y: 0, height: 17 },
      target: { y: 0, height: 17 },
      targetReservations: [8.5]
    })).toBeUndefined();
  });
  ...
  it("builds typed owners, priorities, direct horizontal families, side buckets, and simple Stage gates", async () => {
    ...
    expect(first).toMatchObject({
      ...
      targetEndpoint: { itemId: "J-102", portId: "J-102__flow_in", side: "west", x: 300, y: 116 },
      stageGates: []
    });
    ...
    expect(second).toMatchObject({
      ...
      archetype: "adjacent_forward_same_stage",
      routeFamily: "direct_horizontal",
      sourceEndpoint: { x: 524, y: 116 },
      targetEndpoint: { x: 548, y: 116 }
    });
    ...
    expect(cross).toMatchObject({
      ...
      routeFamily: "direct_horizontal",
      authorOrder: 2,
      sourceEndpoint: { itemId: "J-103", portId: "J-103__flow_out", side: "east", x: 772, y: 116 },
      targetEndpoint: { itemId: "J-201", portId: "J-201__flow_in", side: "west", x: 852, y: 116 },
      stageGates: [
        { stageId: "G-100", side: "east", x: 792, y: 116, order: 0, locked: true },
        { stageId: "G-200", side: "west", x: 832, y: 116, order: 0, locked: true }
      ]
    });
    ...
    for (const bucket of routingStages.nodeEdgeBuckets.filter((candidate) =>
      ["J-101", "J-102", "J-103"].includes(candidate.nodeId)
    )) {
  ...
  it("produces orthogonal exterior routes with legal gates and clear obstacles", async () => {
    const fixture = await buildFixture("primary");
    ...
    for (const plan of fixture.routingStages.connectorPlans.slice(0, 3)) {
      ...
      for (const gate of plan.stageGates) {
        ...
        expect(routeContainsPoint(plan.finalBasicRoute, gate)).toBe(true);
      }
    }
    expect(withoutGate8TerminalDiagnostics(fixture.routingStages.diagnostics)
      .filter((diagnostic) => diagnostic.phase === "routing")).toEqual([]);
  });
  ...
  it("keeps profile-dependent target geometry deterministic without changing the routed family", async () => {
    ...
    expect(simple.routingStages.connectorPlans[2]?.targetEndpoint.y).toBe(116);
    expect(permissive.routingStages.connectorPlans[2]?.targetEndpoint.y).toBe(116);
    expect(strict.routingStages.connectorPlans[2]?.targetEndpoint.y).toBe(116);
  ...
  it("renders deterministic partial step-2 SVG and PNG from the exact same SVG", async () => {
    ...
    expect(first.diagnostics.some(isBlockingJourneyDiagnostic)).toBe(false);
  });
  ...
  it("emits locked structural diagnostics for missing endpoints, omissions, duplicate IDs, and diagonals", async () => {
    ...
    diagonalPlan.finalBasicRoute.points = [
      ...
      { x: diagonalPlan.targetEndpoint.x, y: diagonalPlan.targetEndpoint.y + 1 },
  ...
  it("independently rejects route-family metadata mutations for every selected remediation family", async () => {
    const fixture = await buildFixture("primary");
    const mutations = [
      ["J-101→J-102", "minimal_l"],
      ["J-201→J-203", "direct_horizontal"],
      ["J-204→J-401", "direct_horizontal"]
    ] as const;

    for (const [pair, routeFamily] of mutations) {
      const plan = fixture.routingStages.connectorPlans.find((candidate) => edgeKey(candidate) === pair);
      expect(plan).toBeDefined();
      const mutated = structuredClone(plan!) as JourneyMapConnectorPlan;
      mutated.routeFamily = routeFamily;
    ...
    const directPlan = fixture.routingStages.connectorPlans.find((candidate) =>
      edgeKey(candidate) === "J-101→J-102"
    );
    expect(directPlan).toBeDefined();
    const contextFreeMutation = structuredClone(directPlan!) as JourneyMapConnectorPlan;
    contextFreeMutation.routeFamily = "archetype_template";
    expect(validateJourneyMapRoutes(
      [contextFreeMutation],
      fixture.preRoutingPositionedScene,
      "provisional"
    ...
    const resolvedPlans = structuredClone(
      fixture.routingStages.connectorPlans
    ) as JourneyMapConnectorPlan[];
    resolvedPlans.find((candidate) => candidate.id === directPlan!.id)!.routeFamily =
      "archetype_template";
  ...
  it("keeps the public J-001 to J-002 proof on one direct horizontal segment", async () => {
    const bundle = await bundlePromise;
    const publicPath = path.join(repoRoot, "bundle/v0.1/examples/outcome_to_ia_trace.sdd");
    const compiled = compileSource({
      path: publicPath,
      text: await readFile(publicPath, "utf8")
    }, bundle);
    expect(compiled.diagnostics).toEqual([]);
    expect(compiled.graph).toBeDefined();
    const projected = projectView(compiled.graph!, bundle, "journey_map");
    ...
    expect(plan?.routeFamily).toBe("direct_horizontal");
    expect(plan?.finalBasicRoute.points).toEqual([
      { x: 276, y: 116 },
      { x: 300, y: 116 }
    ]);
    expect(rendered.routingStages.finalPositionedScene.edges.find((edge) =>
      edge.from.itemId === "J-001" && edge.to.itemId === "J-002"
    )?.route.points).toEqual([
      { x: 276, y: 116 },
      { x: 300, y: 116 }
    ]);
  });

  it("keeps balanced direct endpoint reservations deterministic under measured-edge reordering", async () => {
    const fixture = await buildFixture("primary");
    ...
    const directGeometry = (routing: JourneyMapRoutingStages) => routing.connectorPlans
      .filter((plan) => plan.routeFamily === "direct_horizontal")
      .map((plan) => ({
        id: plan.id,
        sourceEndpoint: plan.sourceEndpoint,
        targetEndpoint: plan.targetEndpoint,
        route: plan.finalBasicRoute
      }));
    expect(directGeometry(reversed)).toEqual(directGeometry(fixture.routingStages));
  });
});
...
describe("journey map Gate 7 typed occupancy extraction", () => {
  ...
  it("extracts every locked resource kind with complete recomputable records across all profile-runs", async () => {
    ...
    for (const fixture of fixtures) {
      ...
      const resolvedDiagnostics = validateJourneyMapResolvedStages(
        connectorPlans,
        fixture.routingStages.nominalOccupancy,
        fixture.routingStages.occupancy,
        fixture.routingStages.resolvedConnectors,
      ...
      expect(withoutKnownDenseExperimentDebt(resolvedDiagnostics)).toEqual([]);
  ...
  it("keeps stacked branch and join endpoints distinct across direct and minimal-L sides", async () => {
    const fixture = await buildFixture("primary");
    ...
    expect(nearBranch.finalRoute.points).toEqual([{ x: 1076, y: 116 }, { x: 1100, y: 116 }]);
    ...
    expect(lowerJoin.finalRoute.points).toEqual([{ x: 1324, y: 116 }, { x: 1348, y: 116 }]);
    expect(fixture.routingStages.expansionAttempts).toEqual([]);
    expect(findNode(fixture.routingStages.finalPositionedScene, "J-203").x).toBe(1100);
    expect(findNode(fixture.routingStages.finalPositionedScene, "J-204").x).toBe(1348);
  ...
  it("separates simple reciprocal SCCs as nested arcs while preserving the accepted self-loop", async () => {
    ...
    expect(reciprocalTrackRecords.map((record) => [
      ...
      [planByPair.get("J-701->J-702")!.id, 158, 158],
      [planByPair.get("J-702->J-701")!.id, 158, 174],
      [planByPair.get("J-711->J-712")!.id, 163, 163],
      [planByPair.get("J-712->J-711")!.id, 163, 179]
    ]));
  ...
  it("records the rejected dense early-egress experiment without accepting its visual evidence", async () => {
    ...
    const preferredLegDiagnostics = routingStages.diagnostics.filter((diagnostic) =>
    ...
    expect(routingStages.failedConnectorIds).toEqual([]);
    expect(routingStages.deferredConnectors).toEqual([]);
    expect(routingStages.connectorPlans.map((plan) => plan.id).sort()).toEqual(
      fixture.measuredScene.edges.map((edge) => edge.id).sort()
    );
    expect(routingStages.connectorPlans.some((plan) =>
    ...
    expect(routingStages.finalPositionedScene.root.height).toBeGreaterThan(384);
    expect(routingStages.residualCrossings.length).toBeGreaterThan(31);
    expect(routingStages.expansionAttempts).toHaveLength(2);
    expect(routingStages.expansionAttempts.flatMap((attempt) => attempt.requests).some((request) =>
      request.kind === "stage_progression_gap"
    )).toBe(true);
    ...
    expect(rerun.residualCrossings).toEqual(routingStages.residualCrossings);
  });
  ...
  it("independently rejects every resolved occupancy, reconstruction, and expansion mutation", async () => {
    ...
    expect(withoutKnownDenseExperimentDebt(validate(fixture.routingStages))).toEqual([]);
...
describe("journey map Gate 6 non-adjacent same-Stage routing", () => {
  ...
  it("uses a bounds-derived south-to-south bypass below the Step row without moving accepted geometry", async () => {
    ...
    expect(routeSegments.every(({ start, end }) => segmentLength(start, end) !== undefined)).toBe(true);
    expect(segmentLength(routeSegments[0]!.start, routeSegments[0]!.end)).toBe(
      JOURNEY_MAP_PREFERRED_TERMINAL_LEG
    );
    expect(segmentLength(routeSegments.at(-1)!.start, routeSegments.at(-1)!.end)).toBeGreaterThanOrEqual(
      MIN_ARROW_MARKER_LEG
    ...
    expect(withoutGate8TerminalDiagnostics(fixture.routingStages.diagnostics).filter((diagnostic) =>
  ...
  it("renders deterministic family debug SVG/PNG while retaining accepted earlier route structures", async () => {
    ...
    expect(primary.routingStages.connectorPlans.slice(0, 3).map((plan) => ({
      ...
      {
        edge: "J-101→J-102",
        step2: [{ x: 276, y: 116 }, { x: 300, y: 116 }],
        provisional: [{ x: 276, y: 116 }, { x: 300, y: 116 }]
      },
      {
        edge: "J-102→J-103",
        step2: [{ x: 524, y: 116 }, { x: 548, y: 116 }],
        provisional: [{ x: 524, y: 116 }, { x: 548, y: 116 }]
      },
      {
        edge: "J-103→J-201",
        step2: [{ x: 772, y: 116 }, { x: 852, y: 116 }],
        provisional: [{ x: 772, y: 116 }, { x: 852, y: 116 }]
      }
    ]);
  });
  ...
  it("fails visibly instead of falling back when a south port or baseline bypass space is unavailable", async () => {
    ...
    findContainer(insufficientSpaceScene, "G-500").height = 131;
...
describe("journey map Gate 6 long cross-Stage and root-Step routing", () => {
  it("routes exactly the two locked primary proof occurrences with stable identity and priority", async () => {
    ...
    expect(longCross).toMatchObject({
      ...
      routeFamily: "long_forward_east_egress",
      authorOrder: 7,
      sameEndpointOrdinal: 0,
      exactIdentityOrdinal: 0,
      sourceEndpoint: {
      ...
      stageGates: [
        { stageId: "G-200", side: "east", x: 1592, y: 116, order: 0, locked: true },
        { stageId: "G-400", side: "south", x: 2533.592, y: 160, order: 0, locked: true }
      ],
      rootSpanBypass: {
        ownerContainerId: "root",
        axis: "horizontal",
        nominalCoordinate: 178,
        span: { start: 1610, end: 2533.592 },
        clearanceRootItemIds: ["J-250", "J-260", "G-300", "G-400"],
        intermediateRootItemIds: ["J-250", "J-260", "G-300"],
        obstacleControls: [
          { rootItemId: "J-250", entryX: 1632, exitX: 1856 },
          { rootItemId: "J-260", entryX: 1896, exitX: 2120 },
          { rootItemId: "G-300", entryX: 2160, exitX: 2361.592 }
        ],
        order: 0,
        locked: false
      }
  ...
  it("uses a right-edge east egress and span-local root track while keeping the adjacent root chain direct", async () => {
    const fixture = await buildFixture("primary");
    const [longCross, rootDirect] = fixture.routingStages.connectorPlans.slice(3);
    ...
    const overlappingRootItemBottom = Math.max(
      ...fixture.preRoutingPositionedScene.root.children
        .filter((item) => ["J-250", "J-260", "G-300", "G-400"].includes(item.id))
        .map((item) => item.y + item.height)
    );
    expect(longCross?.rootSpanBypass?.nominalCoordinate).toBe(
      overlappingRootItemBottom + JOURNEY_MAP_PREFERRED_TERMINAL_LEG
    );
    expect(longCross?.rootSpanBypass?.nominalCoordinate).toBeLessThan(
      fixture.preRoutingPositionedScene.root.y + fixture.preRoutingPositionedScene.root.height
    );
    const buckets = new Map(fixture.routingStages.nodeEdgeBuckets.map((bucket) => [bucket.nodeId, bucket]));
    expect(buckets.get("J-204")?.east.startingConnectorIds).toEqual([longCross?.id]);
  ...
  it("derives the span-local track from positioned bounds without changing semantic routing", async () => {
    const simple = await buildFixture("primary", "simple");
    const permissive = await buildFixture("primary", "permissive");
    const strict = await buildFixture("primary", "strict");
    ...
    expect(simpleLong.rootSpanBypass?.nominalCoordinate).toBe(178);
    expect(simpleLong.stageGates.map((gate) => gate.y)).toEqual([116, 160]);
    expect(strictLong.rootSpanBypass?.nominalCoordinate).toBe(178);
    expect(strictLong.stageGates.map((gate) => gate.y)).toEqual([116, 160]);
    expect(simpleLong.rootSpanBypass?.obstacleControls).toEqual(
      strictLong.rootSpanBypass?.obstacleControls
    );
    expect(simple.routingStages.connectorPlans[4]?.step2Route.points).toEqual([
      { x: 1856, y: 116 }, { x: 1896, y: 116 }
    ]);
  });

  it("keeps G-200 dimensions independent of the accepted long cross-Stage edge", async () => {
    const fixture = await buildFixture("primary");
    const withoutLongEdge = structuredClone(fixture.measuredScene) as MeasuredScene;
    withoutLongEdge.edges = withoutLongEdge.edges.filter((edge) =>
      !(edge.from.itemId === "J-204" && edge.to.itemId === "J-401")
    );
    const withoutLongRouting = buildJourneyMapRoutingStages(
      withoutLongEdge,
      fixture.preRoutingPositionedScene
    );
    const dimensions = (scene: PositionedScene) => {
      const stage = findContainer(scene, "G-200");
      return { width: stage.width, height: stage.height };
    };

    expect(dimensions(withoutLongRouting.finalPositionedScene)).toEqual(
      dimensions(fixture.routingStages.finalPositionedScene)
    );
  });

  it("derives span-local clearance only from horizontally overlapping root items", async () => {
    const fixture = await buildFixture("primary");
    const baseline = fixture.routingStages.connectorPlans[3]!;
    expect(baseline.rootSpanBypass?.clearanceRootItemIds).toEqual([
      "J-250",
      "J-260",
      "G-300",
      "G-400"
    ...
    const nonOverlappingTallScene = structuredClone(
      fixture.preRoutingPositionedScene
    ) as PositionedScene;
    findContainer(nonOverlappingTallScene, "G-100").height = 300;
    const nonOverlappingTall = buildJourneyMapRoutingStages(
      fixture.measuredScene,
      nonOverlappingTallScene
    ).connectorPlans.find((plan) => edgeKey(plan) === "J-204→J-401");
    expect(nonOverlappingTall?.rootSpanBypass?.nominalCoordinate).toBe(
      baseline.rootSpanBypass?.nominalCoordinate
    );

    const overlappingTallScene = structuredClone(
      fixture.preRoutingPositionedScene
    ) as PositionedScene;
    findContainer(overlappingTallScene, "G-300").height += 80;
    const overlappingTall = buildJourneyMapRoutingStages(
      fixture.measuredScene,
      overlappingTallScene
    ).connectorPlans.find((plan) => edgeKey(plan) === "J-204→J-401");
    expect(overlappingTall?.rootSpanBypass?.nominalCoordinate).toBe(
      findContainer(overlappingTallScene, "G-300").y
        + findContainer(overlappingTallScene, "G-300").height
        + JOURNEY_MAP_PREFERRED_TERMINAL_LEG
    );
    expect(overlappingTall?.rootSpanBypass?.clearanceRootItemIds).toContain("G-400");
  });

  it("separates overlapping root-span claims downward and requests bounded root expansion", async () => {
    const fixture = await buildFixture("primary");
    const longPlan = fixture.routingStages.connectorPlans[3]!;
    const longState = fixture.routingStages.resolvedConnectors.find((state) =>
      state.connectorId === longPlan.id
    )!;
    const contender = structuredClone(longPlan) as JourneyMapConnectorPlan;
    contender.id = `${longPlan.id}:root-span-contender`;
    contender.authorOrder += 1;
    contender.priority.authorOrder += 1;
    contender.priority.edgeId = contender.id;
    const contenderState = structuredClone(longState);
    contenderState.connectorId = contender.id;
    const plans = [longPlan, contender];
    const states = [longState, contenderState];
    const nominalOccupancy = extractJourneyMapOccupancy(
      plans,
      fixture.preRoutingPositionedScene
    );
    const constrainedScene = structuredClone(
      fixture.preRoutingPositionedScene
    ) as PositionedScene;
    constrainedScene.root.height = 200;
    const resolution = resolveJourneyMapTrackOccupancy(
      plans,
      states,
      nominalOccupancy,
      constrainedScene
    ...
    const spanCoordinates = resolution.resolvedConnectors.map((state) =>
      state.segmentCoordinates.find((coordinate) =>
        ...
        && coordinate.nominalCoordinate === longPlan.rootSpanBypass?.nominalCoordinate
      )?.resolvedCoordinate
    );
    expect(spanCoordinates).toEqual([178, 194]);
    expect(resolution.expansionRequests).toEqual(expect.arrayContaining([
    ...
    expect(resolution.expansionRequests.find((request) =>
      request.kind === "root_span_gutter"
    )?.amount).toBeGreaterThan(0);
  });

  it("diagnoses a full-row east-egress fallback when the local track loses a route comparison", async () => {
    const fixture = await buildFixture("primary");
    const fallbackScene = structuredClone(fixture.preRoutingPositionedScene) as PositionedScene;
    const branchSource = findNode(fallbackScene, "J-201");
    branchSource.x = 1740;
    branchSource.y = 120;
    findNode(fallbackScene, "J-203").y = 116;
    const fallback = buildJourneyMapRoutingStages(fixture.measuredScene, fallbackScene);
    const longPlan = fallback.connectorPlans.find((plan) => edgeKey(plan) === "J-204→J-401");
    expect(longPlan?.routeFamily).toBe("long_forward_east_egress");
    expect(longPlan?.rootSpanBypass).toBeUndefined();
    expect(longPlan?.rootOuterBypass).toBeDefined();
    expect(fallback.diagnostics).toContainEqual(expect.objectContaining({
      code: "renderer.routing.journey_map_span_local_bypass_unavailable",
      severity: "warn",
      targetId: longPlan?.id
    }));
  });

  it("fails visibly instead of reverting to south egress and diagnoses malformed contracts", async () => {
    const fixture = await buildFixture("primary");
    ...
    expect(missingPort.connectorPlans.find((plan) => plan.id === longId)).toBeUndefined();
    expect(missingPort.diagnostics.map((diagnostic) => diagnostic.code)).toEqual(
    ...
    const insufficientSpaceScene = structuredClone(fixture.preRoutingPositionedScene) as PositionedScene;
    insufficientSpaceScene.root.height = 170;
    const insufficientSpace = buildJourneyMapRoutingStages(
      fixture.measuredScene,
      insufficientSpaceScene
    );
    expect(insufficientSpace.failedConnectorIds).toContain(longId);
    expect(insufficientSpace.connectorPlans.find((plan) => plan.id === longId)).toBeUndefined();
    expect(insufficientSpace.diagnostics.map((diagnostic) => diagnostic.code)).toContain(
      "renderer.routing.journey_map_edge_omitted"
    );
    ...
    const misalignedDirect = misalignedRootStep.connectorPlans.find((plan) => edgeKey(plan) === "J-250→J-260");
    expect(misalignedDirect?.routeFamily).toBe("direct_horizontal");
    expect(misalignedDirect?.step2Route.points).toHaveLength(2);

    const malformedGate = structuredClone(fixture.routingStages.connectorPlans[3]!) as JourneyMapConnectorPlan;
    malformedGate.stageGates[0]!.side = "south";
    expect(validateJourneyMapRoutes(
      [malformedGate],
      fixture.preRoutingPositionedScene,
      "provisional"
    ...
    const malformedClearanceIds = structuredClone(
      fixture.routingStages.connectorPlans[3]!
    ) as JourneyMapConnectorPlan;
    malformedClearanceIds.rootSpanBypass!.clearanceRootItemIds.pop();
    const malformedSpan = structuredClone(
      fixture.routingStages.connectorPlans[3]!
    ) as JourneyMapConnectorPlan;
    malformedSpan.rootSpanBypass!.span.start += 1;
    const malformedNominalY = structuredClone(
      fixture.routingStages.connectorPlans[3]!
    ) as JourneyMapConnectorPlan;
    malformedNominalY.rootSpanBypass!.nominalCoordinate += 1;
    const conflictingBypasses = structuredClone(
      fixture.routingStages.connectorPlans[3]!
    ) as JourneyMapConnectorPlan;
    conflictingBypasses.rootOuterBypass = {
      ownerContainerId: "root",
      axis: "horizontal",
      nominalCoordinate: 330,
      span: { start: 1610, end: 2533.592 },
      endpointSpan: { start: 1572, end: 2533.592 },
      intermediateRootItemIds: ["J-250", "J-260", "G-300"],
      obstacleControls: [
        { rootItemId: "J-250", entryX: 1632, exitX: 1856 },
        { rootItemId: "J-260", entryX: 1896, exitX: 2120 },
...
describe("journey map Gate 6 branch fan-out routing", () => {
  it("applies forward source-branch precedence to exactly two primary and three topology occurrences", async () => {
    ...
    expect(withoutGate8TerminalDiagnostics(reordered.diagnostics).filter((diagnostic) =>
      diagnostic.phase === "routing"
    )).toEqual([]);
  });

  it("builds a direct upper branch and a legal two-segment south-to-west lower branch", async () => {
    const fixture = await buildFixture("primary");
    const [direct, lower] = fixture.routingStages.connectorPlans.slice(5, 7);
    expect(direct).toMatchObject({
      routeFamily: "direct_horizontal",
      sourceEndpoint: {
        itemId: "J-201", portId: "J-201__flow_out", side: "east", x: 1076, y: 116
      },
      targetEndpoint: {
        itemId: "J-202", portId: "J-202__flow_in", side: "west", x: 1100, y: 116
      },
      stageGates: []
    });
    expect(direct?.step2Route.points).toEqual([{ x: 1076, y: 116 }, { x: 1100, y: 116 }]);
    expect(direct?.provisionalRoute).toEqual(direct?.step2Route);
    expect(lower).toMatchObject({
      routeFamily: "minimal_l",
      sourceEndpoint: {
        itemId: "J-201", portId: "J-201__escape_out", side: "south", x: 964, y: 193
      },
      targetEndpoint: {
        itemId: "J-203", portId: "J-203__flow_in", side: "west", x: 1100, y: 243.5
      },
      stageGates: []
    });
    ...
    const buckets = new Map(fixture.routingStages.nodeEdgeBuckets.map((bucket) => [bucket.nodeId, bucket]));
    expect(buckets.get("J-201")?.east.startingConnectorIds).toEqual([direct?.id]);
    expect(buckets.get("J-201")?.south.startingConnectorIds).toEqual([lower?.id]);
    expect(buckets.get("J-202")?.west.endingConnectorIds).toEqual([direct?.id]);
    expect(buckets.get("J-203")?.west.endingConnectorIds).toEqual([lower?.id]);
    expect(validateJourneyMapRoutes(
      fixture.routingStages.connectorPlans,
      fixture.preRoutingPositionedScene,
      "step2"
  ...
  it("routes topology fan-out through one-sided gates and a root-peripheral branch track", async () => {
    ...
    expect(enterStage?.provisionalRoute.points).toEqual(enterStage?.step2Route.points);
    ...
    expect(outer?.step2Route.points).toEqual([
      { x: 256, y: 116 }, { x: 276, y: 116 }, { x: 276, y: 183 },
      { x: 1952, y: 183 }, { x: 1952, y: 140 }
    ]);
    expect(outer?.provisionalRoute.points).toEqual([
      { x: 256, y: 116 }, { x: 276, y: 116 }, { x: 276, y: 183 },
      { x: 296, y: 183 }, { x: 1800, y: 183 },
      { x: 1952, y: 183 }, { x: 1952, y: 140 }
    ]);
  ...
  it("derives primary branch geometry across profiles without changing classification", async () => {
    ...
    expect(simple.routingStages.connectorPlans.map(edgeKey)).toEqual(strict.routingStages.connectorPlans.map(edgeKey));
    expect(permissive.routingStages.connectorPlans).toEqual(strict.routingStages.connectorPlans);
    const [simpleDirect, simpleLower] = simple.routingStages.connectorPlans.slice(5, 7);
    expect(simpleDirect?.routeFamily).toBe("direct_horizontal");
    expect(simpleDirect?.step2Route.points).toHaveLength(2);
    expect(simpleLower?.routeFamily).toBe("minimal_l");
    expect(simpleLower?.step2Route.points).toHaveLength(3);
    expect(simpleLower?.sourceEndpoint.side).toBe("south");
    expect(simpleLower?.targetEndpoint.side).toBe("west");
  });
  ...
  it("fails branch construction and validation visibly without activating Gate 7 fallbacks", async () => {
    ...
    expect(insufficientLocal.failedConnectorIds).not.toContain(branchIds[1]);
    expect(insufficientLocal.connectorPlans.find((plan) => plan.id === branchIds[1])?.routeFamily).toBe(
      "archetype_template"
    );

    const topology = await buildFixture("topology");
    const insufficientRootScene = structuredClone(topology.preRoutingPositionedScene) as PositionedScene;
    insufficientRootScene.root.height = 177;
    ...
    malformedDeparture.branch!.sourceOutdegree = 3;
    expect(validateJourneyMapRoutes(
      [malformedDeparture],
      primary.preRoutingPositionedScene,
      "provisional",
...
describe("journey map Gate 6 join fan-in routing", () => {
  it("routes exactly the two authored primary joins and preserves every accepted earlier plan", async () => {
    ...
    expect(joins.map((plan) => ({
    ...
    }))).toEqual([
      {
        ...
        archetype: "adjacent_forward_same_stage",
        modifiers: ["join"],
        join: { targetIndegree: 2, targetOrdinal: 0 },
        rank: 5,
        authorOrder: 5
      },
      {
    ...
    expect(withoutGate8TerminalDiagnostics(reordered.diagnostics)
      .filter((diagnostic) => diagnostic.phase === "routing")).toEqual([]);
  ...
  it("builds a direct upper join and a legal two-segment east-to-south lower join across profiles", async () => {
    const strict = await buildFixture("primary", "strict");
    const [direct, lower] = strict.routingStages.connectorPlans.slice(7);
    expect(direct).toMatchObject({
      routeFamily: "direct_horizontal",
      sourceEndpoint: {
        itemId: "J-202", portId: "J-202__flow_out", side: "east", x: 1324, y: 116
      },
      targetEndpoint: {
        itemId: "J-204", portId: "J-204__flow_in", side: "west", x: 1348, y: 116
      },
      stageGates: []
    });
    expect(direct?.step2Route.points).toEqual([{ x: 1324, y: 116 }, { x: 1348, y: 116 }]);
    expect(direct?.provisionalRoute).toEqual(direct?.step2Route);
    expect(lower).toMatchObject({
      routeFamily: "minimal_l",
      sourceEndpoint: {
        itemId: "J-203", portId: "J-203__flow_out", side: "east", x: 1324, y: 243.5
      },
      targetEndpoint: {
        itemId: "J-204", portId: "J-204__escape_in", side: "south", x: 1460, y: 140
      },
      stageGates: []
    });
    expect(lower?.step2Route.points).toEqual([
    ...
    expect(buckets.get("J-202")?.east.startingConnectorIds).toEqual([direct?.id]);
    expect(buckets.get("J-203")?.east.startingConnectorIds).toEqual([lower?.id]);
    expect(buckets.get("J-204")?.west.endingConnectorIds).toEqual([direct?.id]);
    expect(buckets.get("J-204")?.south.endingConnectorIds).toEqual([lower?.id]);
    for (const stage of ["step2", "provisional", "final_basic"] as const) {
      expect(validateJourneyMapRoutes(
    ...
    const simple = await buildFixture("primary", "simple");
    const permissive = await buildFixture("primary", "permissive");
    expect(simple.routingStages.connectorPlans.slice(7).map((plan) => [
      edgeKey(plan), plan.routeFamily, plan.sourceEndpoint.side, plan.targetEndpoint.side
    ])).toEqual(strict.routingStages.connectorPlans.slice(7).map((plan) => [
      edgeKey(plan), plan.routeFamily, plan.sourceEndpoint.side, plan.targetEndpoint.side
    ]));
  ...
  it("fails malformed joins visibly and defers unsupported join morphologies without Gate 7 behavior", async () => {
    const primary = await buildFixture("primary");
    const [direct, lower] = primary.routingStages.connectorPlans.slice(7);
    const joinIds = [direct!.id, lower!.id];
    ...
    target.ports = target.ports.filter((port) =>
      port.role !== "journey_flow_in" && port.role !== "journey_escape_in"
    );
    ...
    bypassSource.ports = bypassSource.ports.filter((port) => port.role !== "journey_flow_out");
    const missingSourcePort = buildJourneyMapRoutingStages(primary.measuredScene, missingSourcePortScene);
    expect(missingSourcePort.failedConnectorIds).toContain(direct!.id);
    expect(missingSourcePort.failedConnectorIds).not.toContain(lower!.id);
    ...
    expect(insufficientSpace.failedConnectorIds).not.toContain(lower!.id);
    expect(insufficientSpace.connectorPlans.find((plan) => plan.id === lower!.id)?.routeFamily).toBe(
      "archetype_template"
    );

    const malformedDegree = structuredClone(direct!) as JourneyMapConnectorPlan;
    malformedDegree.join!.targetIndegree = 3;
    const swappedOrdinal = structuredClone(direct!) as JourneyMapConnectorPlan;
    swappedOrdinal.join!.targetOrdinal = 1;
    for (const malformed of [malformedDegree, swappedOrdinal]) {
      expect(validateJourneyMapRoutes(
        [malformed],
        primary.preRoutingPositionedScene,
        "provisional",
    ...
    const falseSingleIncomingJoin = structuredClone(direct!) as JourneyMapConnectorPlan;
    ...
    const bypassMeasuredEdge = branchingSourceScene.edges.find((edge) => edge.id === direct!.id)!;
...
describe("journey map Gate 6 backward routing", () => {
  ...
  it("builds south-port Stage-local and reverse root-outer templates with exact controls", async () => {
    ...
    expect(topologyBackward).toMatchObject({
      ...
      stageGates: [],
      stageLocalBypass: {
        stageId: "G-700", nominalCoordinate: 158,
    ...
    const first = await renderJourneyMapRoutingArtifacts(
      ordering.projection, ordering.graph, ordering.bundle, ordering.view, { detailId: "detailed" }
    );
    const second = await renderJourneyMapRoutingArtifacts(
      ordering.projection, ordering.graph, ordering.bundle, ordering.view, { detailId: "detailed" }
    );
  ...
  it("fails malformed backward contracts visibly and keeps duplicates and Gate 7 blocked", async () => {
    ...
    insufficientRoot.root.height = 177;
...
describe("journey map Gate 6 shape-aware cycle routing", () => {
  it("classifies both reciprocal topology components from graph shape without claiming the self-loop", async () => {
    ...
    expect(cyclePlans.map((plan) => plan.provisionalRoute.points)).toEqual([
      [{ x: 428, y: 140 }, { x: 428, y: 158 }, { x: 676, y: 158 }, { x: 676, y: 140 }],
      [{ x: 676, y: 140 }, { x: 676, y: 158 }, { x: 428, y: 158 }, { x: 428, y: 140 }],
      [{ x: 924, y: 140 }, { x: 924, y: 163 }, { x: 1172, y: 163 }, { x: 1172, y: 145 }],
      [{ x: 1172, y: 145 }, { x: 1172, y: 163 }, { x: 924, y: 163 }, { x: 924, y: 140 }]
    ]);
    expect(fixture.routingStages.deferredConnectors).toEqual([]);
    expect(fixture.routingStages.failedConnectorIds).toEqual([]);
    expectExactPartition(fixture);
  ...
  it("applies uniform Alternative B to the complex compressed SCC while retaining full component pressure", async () => {
    ...
    expect(plans.map((plan) => ({
      ...
      stageBypass: plan.stageLocalBypass
        ...
        : undefined,
      rootBypass: plan.rootSpanBypass !== undefined || plan.rootOuterBypass !== undefined
    }))).toEqual([
    ...
    expect([plans[2]!, plans[7]!, plans[12]!, plans[13]!].map((plan) => ({
    ...
    }))).toEqual([
      {
        ...
        departureControl: {
          axis: "vertical", nominalCoordinate: 288, span: { start: 116, end: 158 },
      ...
      },
      {
        ...
        departureControl: {
          axis: "vertical", nominalCoordinate: 536, span: { start: 116, end: 158 },
      ...
      },
      {
        edge: "J-950→J-912",
        role: "ingress",
        span: { start: 1116, end: 1476 },
        intermediateStepIds: ["J-911"],
        obstacleControls: [{ stepId: "J-911", entryX: 1116, exitX: 1340 }],
        departureControl: {
          axis: "vertical", nominalCoordinate: 1086, span: { start: 116, end: 158 },
          obstacleItemId: "G-910", obstacleBoundaryCoordinate: 1116, order: 0, locked: false
        }
      },
      {
        edge: "J-950→J-913",
        role: "ingress",
        span: { start: 1116, end: 1724 },
    ...
    expect(fixture.routingStages.deferredConnectors).toEqual([]);
    expect(fixture.routingStages.failedConnectorIds).toEqual([]);
    expect(withoutGate8TerminalDiagnostics(fixture.routingStages.diagnostics)).toEqual([
      expect.objectContaining({
        code: "renderer.routing.journey_map_preferred_terminal_leg_unmet",
        targetId: "J-903__PRECEDES__J-950__52d993425109eab3b9eefe102c522b22c8c13d129f35ba276e9f93bf496f07e9__0"
      })
    ]);
    expectExactPartition(fixture);
  });
  ...
  it("keeps SCC identity and geometry deterministic under edge reordering and annotation-only changes", async () => {
    ...
    const geometry = (routing: JourneyMapRoutingStages) => routing.connectorPlans.map((plan) => ({
      ...
      rootSpanBypass: plan.rootSpanBypass,
    ...
    expect(withoutGate8TerminalDiagnostics(reversed.diagnostics)).toEqual(
      withoutGate8TerminalDiagnostics(fixture.routingStages.diagnostics)
    );
    expect(withoutGate8TerminalDiagnostics(annotated.diagnostics)).toEqual(
      withoutGate8TerminalDiagnostics(fixture.routingStages.diagnostics)
    );
  });

  it("rejects malformed SCC metadata independently of route-family declarations", async () => {
    const compressed = await buildFixture("compressed");
    ...
    const malformedDiagnostics = validateJourneyMapRoutes(
      malformedPlans,
      compressed.preRoutingPositionedScene,
      "provisional",
      compressed.measuredScene
    );
    expect(malformedDiagnostics.filter((diagnostic) =>
      diagnostic.code === "renderer.routing.journey_map_archetype_fallback"
    ).length).toBeGreaterThanOrEqual(malformedPlans.length);
    expect(validateJourneyMapRoutes(
      [malformedGate],
      compressed.preRoutingPositionedScene,
      "provisional",
  ...
  it("rejects insufficient cycle capacity and does not misclassify reciprocal occurrences", async () => {
    ...
    const topology = topologyCapacity;
...
describe("journey map Gate 8 crossing continuity and terminal diagnostics", () => {
  ...
  it("locks the seven-run diagnostic matrix and marks only residual final crossings", async () => {
    ...
    const codes = (routing: JourneyMapRoutingStages) =>
      routing.diagnostics.map((diagnostic) => diagnostic.code);
    for (const primary of [primarySimple, primaryPermissive, primaryStrict]) {
      expect(codes(primary.routingStages)).toEqual([
        "renderer.scene.journey_map_disconnected_chain"
      ]);
      expect(primary.routingStages.residualCrossings).toHaveLength(0);
    }
    expect(codes(ordering.routingStages)).toEqual([
      "renderer.scene.journey_map_first_parent_selected"
    ]);
    expect(codes(topology.routingStages)).toEqual([
      "renderer.routing.journey_map_peripheral_backward_edge",
      "renderer.routing.journey_map_peripheral_backward_edge",
      "renderer.routing.journey_map_peripheral_backward_edge",
      "renderer.routing.journey_map_peripheral_cycle",
    ...
    for (const fixture of [primarySimple, primaryPermissive, primaryStrict, compressed]) {
      ...
      expect(routing.step3PositionedScene.edges.every((edge) => !edge.continuityMarks)).toBe(true);
      const blockingDiagnostics = routing.diagnostics.filter(isBlockingJourneyDiagnostic);
      if (fixture === compressed) {
      ...
      expect(collectJourneyMapResidualCrossings(
        [...routing.connectorPlans].reverse(),
        [...routing.resolvedConnectors].reverse()
      )).toEqual(routing.residualCrossings);
      for (const crossing of routing.residualCrossings) {
        expect(crossing.edgeAId.localeCompare(crossing.edgeBId)).toBeLessThan(0);
        expect(crossing.overEdgeId).not.toBe(crossing.underEdgeId);
        const warning = routing.diagnostics.find((diagnostic) =>
          diagnostic.code === "renderer.routing.journey_map_unavoidable_crossing"
          && diagnostic.details?.includes(`\"crossingId\":\"${crossing.id}\"`)
        );
  ...
  it("collects strict interior perpendicular crossings but excludes touches and collinear runs", async () => {
    const fixture = await buildFixture("primary");
    const plans = fixture.routingStages.connectorPlans.slice(0, 2);
    const states = fixture.routingStages.resolvedConnectors.slice(0, 2).map((state) =>
    ...
    expect(collectJourneyMapResidualCrossings(plans, states)).toHaveLength(1);

    states[1]!.finalRoute.points = [{ x: 20, y: 0 }, { x: 20, y: 20 }];
    expect(collectJourneyMapResidualCrossings(plans, states)).toEqual([]);
    states[1]!.finalRoute.points = [{ x: 5, y: 10 }, { x: 15, y: 10 }];
    expect(collectJourneyMapResidualCrossings(plans, states)).toEqual([]);
  });
});
