...
- Graphviz, for explicit `legacy_graphviz_preview`, internal `--dot-out`/DOT-backed fallback flows, and VS Code extensions or other tools that shell out to `dot`

Graphviz is not required for the core v0.1 build/test flow or any default staged SVG/PNG preview. It is required for explicit legacy previews and integrations that invoke the Graphviz binary.
...
- `service_blueprint` now also defaults `sdd show` to the staged preview backend `staged_service_blueprint_preview`, which renders the renderer-owned lane/column layout and routed staged SVG directly and derives PNG from that SVG
...
- `journey_map` defaults `sdd show` to `staged_journey_map_preview`, which preserves source-ordered Stage/Step placement, applies dedicated orthogonal routing and continuity marks, and derives PNG from the exact staged SVG
- all six renderable views default to staged preview; `legacy_graphviz_preview` remains explicitly selectable for each, and internal `--dot-out` automatically chooses a DOT-capable backend when needed
...
Render the preserved legacy Graphviz Scenario Flow preview explicitly:

```bash
pnpm sdd show bundle/v0.1/examples/scenario_branching.sdd --view scenario_flow --backend legacy_graphviz_preview --out /tmp/scenario-legacy.svg
```
...
- `src/renderer/staged/uiContracts.ts` exposes the public staged `ui_contracts` SVG/PNG path. `uiContractsPresentationModel.ts` consumes bundle display policy, and the view-owned scene/container modules assemble the B5 hierarchy overview, local scopes, native title bars and references. PNG is derived from the same SVG; legacy rendering keeps its previous model.
- UI Contracts display policy is bundle-owned: compact detail retains component hierarchy and multi-node component scopes while omitting simple local scopes; detailed detail retains those scopes and packs consecutive simple scopes in source order. The presentation model declares eligible scopes, while shared stack layout performs the wrapping and intrinsic container sizing.
- The `ui_contracts_presentation.target_register_layout` bundle policy selects a column or source-ordered rows for the Referenced targets enclosure. Row packing uses the final width of the widest other top-level section, falling back to the widest target card when no other section exists; the shared stack layout keeps the enclosure at that width.
- UI Contracts containers opt into intrinsic bounds so measured estimates cannot leave unused space around laid-out transition ranks. This is a shared layout intent; other views retain their existing minimum-size behavior unless they opt in.
...
- Committed staged-renderer goldens live under `tests/goldens/renderer-stages/` and include stage JSON, deterministic staged SVG, and diagnostic fixtures for accepted migrated views including `journey_map`; they are implementation-contract fixtures, not bundle source-of-truth artifacts.
- The staged pipeline now has shared measurement, renderer-owned macro-layout, deterministic `layered` placement, and a shared routing core. Staged `ia_place_map` uses manual hub/follower grouping, staged `ui_contracts` combines scoped manual layout with renderer-owned layered transition placement and dedicated contract-label lanes, and staged `scenario_flow` uses custom lane-and-band placement plus staged branch routing without ELK or any other external layout engine.
...
- Staged `journey_map` uses source-ordered placement plus dedicated orthogonal routing, real occupancy, bounded expansion, late endpoint ordering, and final continuity marks without an external layout engine. Dense maps remain a documented readability limitation. A later shared-routing task must simplify unobstructed adjacent horizontal connections to one straight segment instead of view-specific doglegs.
