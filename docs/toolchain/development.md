...
- `src/renderer/staged/macroLayout.ts` is the Step 5 macro-layout entry point: recursive `stack`/`grid`/`lanes` placement, container bounds, container-port resolution, branch-local ELK route-hint application, target-biased staged routing when requested, dedicated contract-label lane assignment, and shared arrow-terminal clearance plus target-approach handling.
...
- The staged pipeline now has shared measurement, manual macro-layout, selective `elk_layered` placement, and shared routing strong enough for both proof views. Staged `ia_place_map` keeps deterministic recursive place structure while adding branch-local ELK routing attempts plus shared vertical target-approach normalization for same-chain navigation, while staged `ui_contracts` combines scoped manual layout with selective `elk_layered` transition placement and dedicated contract-label lanes.
