...
`sdd add <document_path> [--node <node_id>] [--bundle <manifest>]` is the human presentation adapter over Guided Addition v1. It discovers the repository root, normalizes the document path, loads the bundle and source, creates one snapshot, and resolves an optional exact starting-node anchor. Without `--node`, the v1 runtime itself offers standalone-node or relationship addition and the starting-node browser. Diagram selection, change, and clearing happen inside applicable guided browse pages; `sdd add` has no `--view` option.
