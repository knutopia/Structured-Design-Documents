...
- Outcome–Opportunity, Service Blueprint and Scenario Flow call `runRoutingLifecycle` with complete route sets, independently resolved endpoints/sides, marker clearance, node/reservation geometry and finite routing bounds. Only its `resolved` result exposes accepted routes; failed candidates are diagnostic-only and public preview creation rejects the error.
- Final resolution validates unchanged and fixed endpoint sections, reconstructs adjacent bends together, and checks every revision against the whole route set. Labels consume accepted routes; emitted routes are checked again against the accepted context. Service rebuilds fallback candidates at final declared ports, and Scenario resolves its existing canvas extent ownership before acceptance.
- `solveRoutingClaims` and the occupancy reconstruction helpers certify supplied assignments only. Journey retains direct assignment with its directional/reciprocal constraints and specialized orchestration; IA and UI retain their own scoped validation/sharing paths. They are not adopters of the new final route-set lifecycle.
- Outcome/Service retain four effective layout expansion passes and Scenario eight, shared between preparation and final repair. These are termination limits, not geometric feasibility proofs. One pass applies measured capacity requirements to the adapter's stable layout baseline. Final candidate/revision bounds and remaining limitations are recorded in the [routing hardening contract decisions](../routing_hardening/design_decisions.md); acceptance evidence is in the [execution ledger](../routing_hardening/implementation_status.md).
...
- ui contracts use a bundle-driven presentation model to separate semantic identities from visual occurrences, expand repeated Component hierarchies deterministically, and assemble local Place/Component scopes with projection-wide State fallback
...
- inside the staged renderer, `ui_contracts` uses intrinsic-width local groups, single-source outgoing contract references, and measured scene ports/reservations for containment bars, transitions and return channels; shared placement/routing remains unchanged
- inside the staged renderer, `scenario_flow` now uses a custom lane-and-band layout with staged branch routing and debug corpus artifacts for pre-routing, edge-side selection, and gutter occupancy; a bounded preparation pass may choose declared south output ports for positioned leftward or downward connectors after whole-scene validation
