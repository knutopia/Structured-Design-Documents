...
- for staged `ia_place_map`, same-chain navigation follows that author-ordered place chain, keeping consecutive sibling `Place` items aligned within the same rendered lower level while preserving true descendant levels; branch-local ELK can improve a local chain region when geometry allows it, but ELK is still not a global chain-layout source of truth
