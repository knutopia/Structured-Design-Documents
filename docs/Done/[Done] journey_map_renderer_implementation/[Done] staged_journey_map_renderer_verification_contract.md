...
Stage-2 direct-horizontal balance contract: the legal shared Y interval is the open overlap of the source-east and target-west side interiors. The nominal Y is the center of the less-tall endpoint; equal-height endpoints use the midpoint between centers. Accepted direct routes reserve both endpoint-side coordinates in canonical route order. A later direct candidate searches the nominal coordinate, then alternating 16px offsets, and selects lexicographically by zero reservation conflicts, distance from the less-tall center, maximum normalized endpoint displacement, total displacement, and coordinate order. If no shared conflict-free Y exists, the direct candidate is rejected rather than reconstructed as a dogleg. Non-direct late endpoint ordering continues to move around locked direct claims.

Stage-2 long-forward balance contract: nominal `long_forward_east_egress` uses a typed root-span bypass. Its horizontal span begins 18px east of the source Stage east gate and ends at the target south-port X. Its clearance set contains direct root children with strict positive horizontal overlap against that span, in root order; boundary touching is not overlap. The nominal track is the maximum bottom of the clearance set plus 18px. The target Stage participates when it overlaps; a non-overlapping source Stage does not. Span-local claims share a root-and-span occupancy resource, retain 16px separation, and move only downward from nominal. A full-row east-egress candidate may replace the local candidate only to satisfy a higher-priority geometry, separation, or crossing invariant and must emit `renderer.routing.journey_map_span_local_bypass_unavailable`. It must not silently revert to source-south egress. Exactly one span-local or full-row root bypass contract may be present on a connector plan.
...
| Less-tall-center direct Y plus deterministic 16px reservations | step-2, step-3, final | stage-2 primary and public direct proofs |
| Span-local root-bypass clearance and diagnosed full-row fallback | step-2, step-3, final | stage-2 `J-204→J-401` proof and constructed fallback |
...
| `renderer.routing.journey_map_span_local_bypass_unavailable` warn | a full-row east-egress candidate replaces an unavailable or lower-ranked span-local candidate | `[edgeId]` plus the local span when constructible; capacity; fails the nominal stage-2 proof |
